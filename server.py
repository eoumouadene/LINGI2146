...
def leastSquare(yvalue,idnode):
    ...
    if a >=30:
        conn.sendall((str(idnode)+':'+"OpenValve"+'\n').encode())

    else:
        conn.sendall((str(idnode)+':'+"CloseValve"+'\n').encode())
...
while(True):
    ...
    with conn:
        ...
        #nbrdata is the number of times we added a value for each node id 
        memorydata = []
        whichindex=[]
        nbrdata = [0]*100
        
        for i in range(0, 100): #we expect not having more than 100 nodes
            memorydata.append([])#the list of data for each node (nbr max of data==30)
        ...
        while True:
            ...
            if data:
                ...
                if (int(dataTested[0])!=-1): #data to add to memorydata
                    ...
                    if (nbrdata [indexid]==31):#the limit is broken we have to sup the oldest data     
                        nbrdata [indexid] -=1 
                        ...
                        conn.sendall(("Server received the "+str(nbrdata[indexid])+"/30 values from id "+str(int(dataTested[0]))+' \n').encode())                
